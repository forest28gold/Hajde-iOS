...
@interface HDMyCommentsViewController () <UITableViewDragLoadDelegate, UIGestureRecognizerDelegate>
{
    int _dataCount;
    int tempCommentsCount;
}
...
@implementation HDMyCommentsViewController
...
- (void)viewDidLoad {
    ...
    if ([GlobalData sharedGlobalData].g_userInfo.commentCount > 0) {
        SVPROGRESSHUD_SHOW;
...
- (void)initLoadMyVotesData {
    ...
    query.whereClause = [NSString stringWithFormat:@"%@ LIKE \'%%%@%%\'", KEY_COMMENT_TYPE, [GlobalData sharedGlobalData].g_userInfo.userID];
    ...
    [[backendless.persistenceService of:[Post class]] find:query response:^(BackendlessCollection *posts) {
        ...
        if (posts.data.count > 0) {
            ...
            if ([GlobalData sharedGlobalData].g_userInfo.commentCount < tempCommentsCount) {
                [GlobalData sharedGlobalData].g_userInfo.commentCount = tempCommentsCount;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [GlobalData sharedGlobalData].g_postData = self.commentDataArray[indexPath.row];
