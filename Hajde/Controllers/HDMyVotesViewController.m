...
@interface HDMyVotesViewController () <UITableViewDragLoadDelegate, UIGestureRecognizerDelegate>
{
    int _dataCount;
    int tempVotesCount;
}

@property (nonatomic, strong) NSMutableArray *myVotesDataArray;

@end

@implementation HDMyVotesViewController
...
- (void)viewDidLoad {
    ...
    if ([GlobalData sharedGlobalData].g_userInfo.voteCount > 0) {
        SVPROGRESSHUD_SHOW;
        [self initLoadMyVotesData];
    } else {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    } else if (![GlobalData sharedGlobalData].g_toggleMyPostIsOn && ![GlobalData sharedGlobalData].g_togglePhotoIsOn) {
        [self finishRefresh];
...
- (void)initLoadMyVotesData {

    self.myVotesDataArray = [[NSMutableArray alloc] init];
    tempVotesCount = 0;
    ...
    query.whereClause = [NSString stringWithFormat:@"%@ LIKE \'%%%@%%\'", KEY_LIKE_TYPE, [GlobalData sharedGlobalData].g_userInfo.userID];
    ...
    [[backendless.persistenceService of:[Post class]] find:query response:^(BackendlessCollection *posts) {
        ...
        if (posts.data.count > 0) {
            ...
            for (Post *post in posts.data) {
                ...
                [self.myVotesDataArray addObject:record];
                tempVotesCount ++;
                
            }
            
            if ([GlobalData sharedGlobalData].g_userInfo.voteCount < tempVotesCount) {
                [GlobalData sharedGlobalData].g_userInfo.voteCount = tempVotesCount;
...
- (void)onSetEmptyView {
    ...
    if (self.myVotesDataArray.count > 0) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.myVotesDataArray.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    PostData *postData = self.myVotesDataArray[indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PostData *dataDict = self.myVotesDataArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [GlobalData sharedGlobalData].g_postData = self.myVotesDataArray[indexPath.row];
...
- (void)onLikePost:(id)sender {
    ...
    [HDUtility likePostInBackground:indexPath dataArray:self.myVotesDataArray];
...
- (void)onDislikePost:(id)sender {
    ...
    [HDUtility dislikePostInBackground:sender indexPath:indexPath dataArray:self.myVotesDataArray];
