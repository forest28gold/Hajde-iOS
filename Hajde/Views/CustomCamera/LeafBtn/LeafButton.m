...
//
#define COLOR_MAIN       [UIColor redColor]

#import "LeafButton.h"
#define LINEWIDTHRATE 0.05f
...
//        _inCircleLayer.path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(frame.size.width/2, frame.size.height/2) radius:(frame.size.width-lineWidth)/2-2 startAngle:0 endAngle:M_PI*2 clockwise:NO].CGPath;
        _inCircleLayer.frame = CGRectMake(self.lineWidth+1, self.lineWidth+1, frame.size.width-(self.lineWidth+1)*2, frame.size.height-(self.lineWidth+1)*2);
...
-(void)setType:(LeafButtonType)type{
    ...
    if(_type != type){
        ...
        if(type == LeafButtonTypeCamera){
            ...
            animation.toValue = (id)COLOR_MAIN.CGColor;
            [self.inCircleLayer addAnimation:animation forKey:@"backgroundColor"];
            self.inCircleLayer.backgroundColor = COLOR_MAIN.CGColor;
        }
